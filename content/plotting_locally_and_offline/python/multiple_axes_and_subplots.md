...
Below is the same data as for exercise 1, but for France.

1. In your two subplots from above, add the french data.
2. Use the following trace attributes to bind the two countries accross your subplots:
  3. The colors of the lines of each trace can be made identical for the same country (hint: ```line = go.Line()```).
  4.  ```legendgroup``` - takes a string, and each trace with an identical string will be grouped together in the legend.  On clicking, the whole group is toggled together too.
